...
namespace nlohmann
{
    ...
    struct adl_serializer<Reservation>
    {
        ...
        static void from_json(const json &j, Reservation &reserve)
        {
            ...
            for (int i = 0; i < Storage::instance().allMeals.size(); i++)
                if (Storage::instance().allMeals[i].getMealID() == meal_id)
                    reserve.setMeal(Storage::instance().allMeals[i]);
                else
            ...
            for (int i = 0; i < Storage::instance().allDiningHalls.size(); i++)
                if (Storage::instance().allDiningHalls[i].getHallID() == dhall_id)
                    reserve.setDiningHall(Storage::instance().allDiningHalls[i]);
                else
